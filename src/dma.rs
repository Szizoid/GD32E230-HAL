...
//! let transfer = channels.ch1.write_to(usart, b"hello", Prio::Low);
//! let (ch1, usart, _) = transfer.wait();
//! ```
...
// would let either choice compile while one is wired. Requests with no impl yet
    ...
    P: DmaPeriph<N>,
{
    ...
    ///
    /// Does not check `ERRIF`: every address this API can hand a channel comes
    /// from a typed source, so a bus error is unreachable through safe code.
    ...
    /// looks at it, so it serves progress reporting.
